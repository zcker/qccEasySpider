...
Spider-main ├─ constant.py 常量，有正则有requests请求解析 ├─ dist 放生成exe的地方 │ ├─ enterprise_search.txt │ ├─ info.xls │ ├─ main.exe
│ └─ test.txt ├─ enterprise_search.txt 读取搜索 ├─ img.ico ├─ info.xls 输出 ├─ main.py 主函数 ├─ main.spec ├─ spyder.py 爬取数据 ├─
test.txt └─ write_to_excel.py 输出到Excel
